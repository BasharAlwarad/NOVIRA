...
6. Once the user provides their email, create the Neon record with **minimal data** — just the email, plus the Tier 1 answers for continuity (**built** — see `CLAUDE.md`). This reduces risk and the scope of what needs disclosing, but doesn't eliminate legal obligations outright: an email address is still personal data. At this exact point, add a short, honest **privacy notice** next to the field (what's collected, why, how long it's kept, that deletion can be requested) — light enough not to need the Phase 0 lawyer for this specific piece, but it should exist before this ships to real users.

### The signup boundary — see real matches, still free

**New, planned.** To see the *actual* matched opportunities (not just the count from step 4), the user creates a real account — the first point this funnel requires signup/auth, which doesn't exist yet as a system (a genuine build prerequisite, not just a policy statement). Still free, still no documents. Two things must ship together with this:
- **An explicit, honest disclaimer next to the match list**: these are matched against the user's *self-reported, unverified* answers, and accuracy is limited until documents are actually checked — framed plainly, not buried in fine print, consistent with the wording discipline in §3.
- This is also the natural point to introduce the **willingness-to-pay ask** for the next tier (see §5) — as an ask/interest-capture, not real payment processing yet, per the existing sequencing below.
...
2. **Opportunities database** — curated, not scraped, and **University/Ausbildung only for now** — employment/job matching is explicitly excluded (real-time job-posting data is a harder, less stable data problem than semester-based program data; revisit later). **Schema and admin review flow are built** (`backend/Models/Opportunity.cs`, `/admin/opportunities`, see `CLAUDE.md`), seeded with 10 obviously-fake entries for the blueprint pass — reverted from `Approved` to `Denied` on 2026-08-10 once `/opportunity-counts` went live to real visitors, so no fake row is ever counted. Data sources:
   ...
   - Target: 20–50 real, verified opportunities is enough to start (Ausbildung side already there — 25 real listings synced on the first run). Data gathering + human review cadence: roughly once a day or less — program-level data doesn't change often, so the actual effort is building an accurate first dataset, not the refresh frequency.
...
- Phase 2 opportunities database + admin review, and matching/ranking logic, both **built and proven against fake seed data + synthetic profiles** (item 3 above), and **now wired into the user-facing result page** as the real opportunity count (§4 step 4 above) — `/admin/match-preview` remains the separate internal admin-key-protected test harness (returns full match details, not just counts). Real data sourcing (Ausbildung API done, hand-curated university list still pending) is the next unblocked step.
- Missing for Tier 2 entirely: signup/auth, specific document-consent flow (naming the international transfer), document upload + extraction — all deliberately still out of scope, see §11 Phase 2.
- Backend: ASP.NET with `User` + `Opportunity` models, `Services/MatchingService.cs`, Postgres (Neon, **Frankfurt/EU region**) — `POST /leads`, `/admin/opportunities` (GET/PATCH), `/admin/match-preview` (POST), and `POST /opportunity-counts` (public, rate-limited) are real, working endpoints (`backend/Program.cs`, `backend/Endpoints/`, `backend/Services/`).
...
- [x] Create the Neon record (Frankfurt/EU) only once a user provides their email — `POST /leads`, idempotent on normalized email
...
- [ ] **New: signup/auth** — the boundary to see real match *details* (not just the count). No accounts system exists yet; this is a real build item, not just a policy change. Ships together with an explicit "self-reported, unverified" disclaimer next to the match list.
