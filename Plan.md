...
2. **Opportunities database** — curated, not scraped, and **University/Ausbildung only for now** — employment/job matching is explicitly excluded (real-time job-posting data is a harder, less stable data problem than semester-based program data; revisit later). **Schema and admin review flow are built** (`backend/Models/Opportunity.cs`, `/admin/opportunities`, see `CLAUDE.md`), currently seeded with 10 obviously-fake entries — real data sourcing is intentionally on hold until item 3 below is validated. Confirmed data sources for when that happens (researched 2026-07-31):
...
3. **Matching/ranking — the active next step (decided 2026-07-31), ahead of both real data sourcing and document upload.** Starts rule-based (language level, credential-recognition status, budget) with an LLM only for fuzzy explanation/summarization. Every claim the AI makes should trace back to a real source (an actual posting, an actual university's stated requirement) — never a free-floating LLM claim, given the stakes for users. This is the same anti-fabrication discipline already applied to `PlaceholderOpportunityCounts.tsx` and `occupation-demand.ts`. Build and prove this against the existing fake `Opportunity` data and a synthetic user profile first — cheaper to discover a schema gap against fake seed data than against a live external sync, and it tells us definitively whether the opportunities schema (§8 above) is actually sufficient before investing in real data sourcing.
...
- Tier 1 **fully complete**: email capture (`SaveResultPrompt.tsx` → `POST /leads`, branded Resend email), privacy notice, and `Users` row creation are all built and verified end to end.
- Phase 2 opportunities database + admin review **built with fake seed data**; matching/ranking logic is the active next step (item 3 above), ahead of both real data sourcing and document upload.
...
- Backend: ASP.NET with `User` + `Opportunity` models, Postgres (Neon, **Frankfurt/EU region**) — `POST /leads` and `/admin/opportunities` (GET/PATCH) are real, working endpoints (`backend/Program.cs`, `backend/Endpoints/`).
...
- [ ] **Next: build the matching/ranking layer against the existing fake data + a synthetic user profile** (rule-based + LLM explanation, never free-generated institution names — §8) — deliberately sequenced *before* real data sourcing below, so schema gaps surface against fake seed data, not a live sync.
- [ ] Only after that's validated: build the curated University + Ausbildung opportunities database for real — see §8 for confirmed data sources. Jobs/employment explicitly excluded for now.
