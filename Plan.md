...
- **Document upload + AI extraction + human review — built and verified end-to-end 2026-08-15, since substantially extended** (`backend/Services/DocumentVerificationService.cs`, `backend/Endpoints/DocumentsEndpoints.cs`/`DocumentsAdminEndpoints.cs`, `/account` + `/admin/users/profiles`), against synthetic/the founder's own documents only — see CLAUDE.md's "Document verification pipeline" section (and the sections that follow it) for the full design and the real test results (it correctly caught a document-type mismatch and, on a real passport, expiry + a likely internet-sourced watermark). The human-review half (Approve/Deny/FlagRed) has since been exercised live many times over, and the pipeline now also promotes approved document data into a verified profile layer that feeds real matching, plus in-app messaging for decisions and an admin data-consistency check — all built 2026-08-16 through 2026-08-27, see CLAUDE.md. Still genuinely missing for Tier 2, unchanged: the specific document-consent flow (naming the international transfer) needed before this opens to real user submissions — see §11 Phase 2.
