...
**Gap worth flagging, not yet fixed**: we don't currently ask **whether the person already has a job offer or employer contact in Germany**. This materially changes the Employment-path calculus (§2.3 — the €13,092 financial-proof requirement only applies to the *job-seeker* Opportunity Card route, not someone with a contract already in hand) and we have no field for it. Flagging for a decision, not adding unilaterally.
