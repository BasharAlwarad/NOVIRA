...
- [x] Currently seeded with 10 obviously-fake entries (`backend/Data/OpportunitySeeder.cs`, loudly marked `[FAKE]` — same discipline as `PlaceholderOpportunityCounts.tsx`) to validate the approve/deny flow before real data sourcing starts.
- [ ] **Next up: the matching/ranking logic itself — build and prove this against the existing fake data before touching real data sync or document upload.** Take a sample user profile (a synthetic `AssessmentAnswers`, reusing the kind of test profiles already used to validate `assessment-verdict.ts`) and write the comparison logic against the 10 seeded `Opportunity` rows. Rationale (decided 2026-07-31): if the schema turns out to be missing something matching actually needs, that's far cheaper to fix against fake seed data we fully control than against a live external sync. This also directly de-risks the other two candidate next steps rather than guessing which to build first.
- [ ] Only after matching logic is validated: wire up real Ausbildung data — Bundesagentur für Arbeit's "Jobsuche" API filtered to `angebotsart=4` — unofficial/community-documented (`bundesAPI/jobsuche-api` on GitHub), no published ToS/rate limit, fine for prototype, not a committed production dependency.
- [ ] Also after matching logic is validated: university data — no self-serve API exists (Hochschulkompass/HRK requires becoming a "collaborative partner" via direct outreach — a business action item, not a code task; DAAD's programme database has no public API either). Interim: hand-curate a starter set the same way `occupation-demand.ts` is curated.
